...
class EstimateNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, estimate_bound, learning_rate):
        ...
        self.layer_node_1 = 100
        self.layer_node_2 = 200
        self.layer_node_3 = 400
        self.weight_estimate = tf.constant([1., 1., 1., 1.])
        self.weight_target = tf.constant([4.5, 1., 1., 1.])
        ...
        self.loss_target = tf.reduce_mean(tf.multiply(tf.multiply(self.error_target, self.error_target), self.weight_target))
    ...
    def create_estimate_network(self, name):
        ...
        with tf.name_scope(name):
            ...
            with tf.name_scope('fc4'):
                w_fc4 = self.weight_variable([self.layer_node_3, self.state_dim])
                b_fc4 = self.bias_variable([self.state_dim])

                outputs = tf.subtract(tf.nn.sigmoid(tf.matmul(h_fc3, w_fc4) + b_fc4), self.output_range)
